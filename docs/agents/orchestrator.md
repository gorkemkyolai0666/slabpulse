...
Production applications live in dedicated GitHub repositories — not in the factory repository.

After provisioning via `scripts/provision-project.sh`:

The only source of truth is the generated project repository root.

Never implement production code inside the factory repository (`agent-to-agent`).

Legacy monorepo projects under `projects/<project_name>/` in the factory repo are read-only archives.
